...
class CryptoManager:
    ...
    def _derive_secret_key(*, secret_key: str) -> bytes:
        return hashlib.sha256(string=secret_key.encode("utf-8")).digest()  # type: ignore

    @classmethod
    def encrypt(cls, *, data: str, secret_key: str) -> str:
        secret_key_bytes = cls._derive_secret_key(secret_key=secret_key)
        iv = os.urandom(12)
        encryptor = Cipher(
        ...
        ciphertext = encryptor.update(data=data.encode("utf-8")) + encryptor.finalize()  # type: ignore
        encrypted_data = iv + encryptor.tag + ciphertext  # type: ignore
        return base64.urlsafe_b64encode(s=encrypted_data).decode("utf-8")  # type: ignore
